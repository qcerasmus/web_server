...
{
    asio::async_write(*_socket, asio::buffer(_request),
        [&](const std::error_code &ec, const std::size_t length)
        {
            asio::async_read(*_socket, asio::dynamic_buffer(_response),
                [&](const std::error_code &ec, const std::size_t length) { still_running = false; });
        });
}
