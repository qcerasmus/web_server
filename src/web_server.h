...
class web_server
{
  public:
     ...
     */
    explicit web_server(std::string_view ip_address, short port = 80, std::string_view certificate_file = "",
     ...
     * \param function A lambda that will be called once the METHOD and url is
     * hit.
     */
    void register_function(METHODS method, const std::string &url,
        const std::function<void(const web_request &, web_response &)> &function);
    /**
  ...
  protected:
     ...
     * When a client connects, we create a connection object and add it to the
     * _connections vector.
     */
  ...
  private:
     ...
     */
    std::map<METHODS, std::vector<std::pair<std::string, std::function<void(const web_request &, web_response &)>>>>
        _functions;
    /**
...
inline web_server::web_server(const std::string_view ip_address, const short port,
    const std::string_view certificate_file, const std::string_view key_file)
    : _ip_address(ip_address), _port(port),
#ifdef OPEN_SSL
      _io_context(asio::ssl::context::sslv23),
#endif
      _certificate_file(certificate_file), _key_file(key_file)
{
    try
    {
#ifdef OPEN_SSL
        _io_context.set_options(asio::ssl::context::default_workarounds | asio::ssl::context::no_sslv2);
...
inline bool web_server::start()
{
    ...
    std::cout << "[web_server] Started... on port: " << _port << std::endl;
    return true;
}

inline void web_server::register_function(const METHODS method, const std::string &url,
    const std::function<void(const web_request &, web_response &)> &function)
{
        ...
        [&](const std::error_code ec, asio::ip::tcp::socket socket)
        {
            ...
            if (!ec)
            {
                ...
                auto new_connection = std::make_shared<connection>(
                    asio::ssl::stream<asio::ip::tcp::socket>(std::move(socket), _io_context));
#else
