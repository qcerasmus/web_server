...
int main()
{
    try
    {
        ...
        ws.register_function(METHODS::GET, "/",
            [&](const web_request &request_header, web_response &response)
            {
        ...
        ws.register_function(METHODS::GET, "/api/asdf",
            [&](const web_request &request_header, web_response &response)
            {
                response.status_code = 200;
                response.status = "OK";
        ...
        ws.register_function(METHODS::POST, "/api/post_test",
            [&](const web_request &request_header, web_response &response)
            {
